...
**Trainer Interface (Prototype)**: A separate UI for personal trainers to manage multiple clients. Role selection occurs at login via RoleSelectionDialog, storing role in sessionStorage. Routes: `/trainer` (dashboard) and `/trainer/client/:id` (client detail). Dashboard displays client roster cards with avatar, name, status, reactive alert/message counts, last workout time, and goals. Client detail page features 5 tabs: Profile (goals, schedule, equipment), Program (full workout list with completion status, click-to-view workout details), Progress (stats and recent activity), Messages (chat interface with stateful message sending), and Alerts (client feedback with resolution tracking). Mock data managed by TrainerDataProvider context includes 3 clients, full workout programs, exercise feedback (pain/dislike/weight issues), and message threads. Messaging and feedback systems are stateful during session. Currently UI prototype only - no backend integration.
