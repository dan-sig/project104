...
**Trainer Interface (Prototype)**: A separate UI for personal trainers to manage multiple clients. Role selection occurs at login via RoleSelectionDialog, storing role in sessionStorage. Routes: `/trainer` (dashboard) and `/trainer/client/:id` (client detail). Dashboard features a comprehensive table-based client roster with 11 columns (Name, Program, Status, Days/Week, Last Workout, Completion %, Messages, Alerts, Next Workout, Joined Date, Streak) plus advanced filtering (search by name/email, program type dropdown, status dropdown, alert status dropdown, last workout date range picker) with clear-all functionality. Click any row to navigate to client detail. Client detail page features 5 tabs: Profile (goals, schedule, equipment), Program (expandable workout list showing exercises inline, "Change Program" button to switch templates, click any exercise to edit parameters via side drawer), Progress (stats and recent activity), Messages (chat interface with stateful message sending), and Alerts (client feedback with resolution tracking). Program editing includes ChangeProgramDialog (modal for selecting new Morphit program templates) and ExerciseEditorDrawer (side drawer with inputs for exercise swap, tempo, sets, reps, weight, RPE, RIR, rest). Mock data managed by TrainerDataProvider context with full state management includes 3 clients, flattened workout array with clientId field, computed fields (completion %, next workout, streak), exercise feedback (pain/dislike/weight issues), message threads, and logged workout performance data. Context provides editing functions: updateClientProgram, updateExerciseParams, and swapExercise. All changes are stateful during session. **Automated Performance Tracking**: The system automatically analyzes logged workout data (actual weight/reps performed vs prescribed parameters) and generates performance alerts. Five alert types: underperformed_weight (used less weight than prescribed), underperformed_reps (completed fewer reps), incomplete_set (skipped or logged 0 reps), overperformed_weight (exceeded prescribed weight), and overperformed_reps (exceeded prescribed reps). Alerts are generated once on mount using useEffect with useRef guard to prevent StrictMode duplicates, persisted to state alongside manual feedback, and fully resolvable. UI displays alerts with visual differentiation (green borders/backgrounds for overperformance achievements, red for underperformance issues), prescribed vs actual values, and set-level details. Alert counts in roster table reflect both manual and automated alerts. Currently UI prototype only - no backend integration.
