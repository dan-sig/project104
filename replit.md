...
**Cycle-Specific Training Parameters**: Workout generation applies cycle-specific parameters (tempo, RPE, rest, sets/reps) defined in `shared/cycleConstants.ts`. Role-aware programming preserves distinct volume prescriptions for primary compounds, secondary compounds, isolations, and core accessories while sharing cycle-driven tempo/RPE/rest across all exercise types. Power exercises maintain explosive tempo (1-0-X-0) but use cycle-specific rest periods.
